(function(){
  ...
  var demos={bench:'0025',dbrow:'0293',incline:'0314',ezrow:'1344',skull:'0060',lateral:'0334',curl:'0447',pushup:'0662',squat:'0043',rdl:'1459',bulgarian:'0410',stepup:'0431',hip:'3562',calf:'0417',legraise:'0620',plank:'2135',ohp:'1457',invrow:'0499',seated:'0405',pullover:'0375',incurl:'0318',reardelt:'2292',triover:'0430',dips:'0813',deadlift:'0032',goblet:'1760',lunge:'0381',bridge:'3013',situp:'3670',wallsit:'0624'};
  var workouts=[
  ...
  function demoUrl(item){return 'https://workoutxapp.com/gifs/'+demos[item.id]+'.gif';}
  function card(person,item,block){var old=last(state.history[key(person,item)]||[]),kg=weight(person,item,block.sets),rr=reps(person,item),isDone=state.done[person],recommend=old&&suggested(person,item,block.sets)>old.weight,previous=old?'<div class="previous"><span>Minule</span><b>'+fmt(old.weight)+' kg × '+old.reps+'</b>'+(recommend?'<em>+ '+fmt(item.step)+' kg</em>':'')+'</div>':'';return '<article class="card '+person+(isDone?' completed':'')+'"><div class="person"><span class="avatar">'+(person==='son'?'S':'T')+'</span><h2>'+(person==='son'?'Syn':'Táta')+'</h2><span class="station">'+(isDone?'HOTOVO':esc(item.equip))+'</span></div><div class="exercise-visual"><span class="visual-fallback">DG</span><img src="'+demoUrl(item)+'" alt="Animovaná ukázka cviku '+esc(item.name)+'"><button class="tech-button" data-action="help" data-person="'+person+'">Technika</button><span class="demo-credit">WorkoutX</span></div><section class="exercise"><h3>'+esc(item.name)+'</h3><div class="exercise-meta"><b>'+block.sets+' × '+esc(item.label)+'</b>'+previous+'</div></section><div class="controls"><div class="control"><span>VÁHA</span><div class="stepper"><button data-action="weight-down" data-person="'+person+'" aria-label="Snížit váhu">−</button><b>'+fmt(kg)+'<small>kg</small></b><button data-action="weight-up" data-person="'+person+'" aria-label="Zvýšit váhu">+</button></div></div><div class="control"><span>OPAKOVÁNÍ</span><div class="stepper"><button data-action="reps-down" data-person="'+person+'" aria-label="Snížit opakování">−</button><b>'+rr+'</b><button data-action="reps-up" data-person="'+person+'" aria-label="Zvýšit opakování">+</button></div></div></div><button class="done-button" data-action="done" data-person="'+person+'" '+(isDone||state.rest!==null?'disabled':'')+'>'+(isDone?'Série dokončena':'Dokončil jsem sérii')+'</button></article>';}
  function modal(ctx){var html='',i,item,list;if(state.help){item=ctx[state.help];list=tips[item.kind]||tips.press;html='<div class="shade" data-action="close"><div class="modal technique-modal" data-stop="1"><button class="close" data-action="close">×</button><small>TECHNIKA</small><h2>'+esc(item.name)+'</h2><div class="modal-demo"><img src="'+demoUrl(item)+'" alt="Ukázka provedení '+esc(item.name)+'"><span>WorkoutX</span></div><ul class="tips">';for(i=0;i<list.length;i++){html+='<li><b>'+(i+1)+'</b>'+esc(list[i])+'</li>';}html+='</ul><button class="primary" data-action="close">Jdu cvičit</button></div></div>';}
