(function(){
  ...
  function modal(ctx){var html='',i,item,list;
    ...
    if(state.help){item=ctx[state.help];list=tips[item.kind]||tips.press;html='<div class="shade" data-action="close"><div class="modal technique-modal" data-stop="1"><button class="close" data-action="close">×</button><small>PROVEDENÍ CVIKU</small><h2>'+esc(item.name)+'</h2><div class="modal-demo"><img src="'+demoUrl(item)+'" alt="Ukázka provedení '+esc(item.name)+'"><span>WorkoutX</span></div><ul class="tips">';for(i=0;i<list.length;i++){html+='<li><b>'+(i+1)+'</b>'+esc(list[i])+'</li>';}html+='</ul><button class="primary" data-action="close">Rozumím</button></div></div>';}
